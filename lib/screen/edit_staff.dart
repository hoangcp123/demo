...
class EditStaff extends StatefulWidget {
  ...
  const EditStaff({required this.staff, Key? key}) : super(key: key);
...
class _EditStaffState extends State<EditStaff> {
  ...
  void initState() {
    super.initState();
    staffIdController.text = widget.staff.staffId.toString();
    staffNameController.text = widget.staff.staffName ?? '';
    staffDepartmentController.text = widget.staff.department ?? '';
    staffPositionController.text = widget.staff.position ?? '';
  }
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      if (!validateStaffId && !validateStaffName && !validateStaffDepartment && !validateStaffPosition) {
                        ...
                        var result = await staffService.updateStaff(staff);
                        if (result != null) {
                          showAlertDialog('Notification', 'Updated staff successfully');
                        } else {
                          showAlertDialog('Notification', 'Failed to update staff');
                        }
                      }
                    },
